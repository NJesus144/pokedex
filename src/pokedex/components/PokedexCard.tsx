...
const isFavorite = favorites.some((poke)=> poke.name === pokemon.name)
    return (
        <div>
            <div>  
            <div  className="styleCssCard border-2 flex justify-between cursor-pointer shadow-lg">                     
               <div  onClick={handleClick} className=" p-4 flex items-center ">                          
                    ...
                    <div className='flex flex-col items-start capitalize'>
                       {pokemon.name}       
                        <div className=' mt-4 flex '>{pokemon.types.map((type)=><div className='mr-2 border-2 px-6 rounded-md'>{type.type.name}</div> )}</div>
                    </div>
                </div>
